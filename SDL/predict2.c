...
predict_cascade_t* load_adaboost(const char* file) {
  ...
  while (fgets(line, 255, fp) != NULL) {
    ...
    int h;
    int charac;
    int n = sscanf(line, "%d feature=%d alpha=%lf threshold=%lli parity=%d "
                   "j=%d i=%d w=%d h=%d charac=%d", &t,
                   &feature, &alpha,
                   &threshold, &parity, &j, &i, &w, &h, &charac);
    printf("n : %d\n", n);
    if (n != 10) {
      double strong_threshold;
      int n = sscanf(line, "layer=%d strong_threshold=%lf nb_features=%d", &(cascade->nb_layers), &strong_threshold, &t);
      printf("n : %d\n", n);
